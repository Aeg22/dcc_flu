...
#gseaplot2_ag(edo2, geneSetID = ids,
#          base_size = 11,
#          subplots = 1:2)
#gseaplot2_ag(edo2, geneSetID = ids,
#             base_size = 11,
...
gseaplot2_ag <- function (x, geneSetID, title = "", color = "green", base_size = 11, 
          rel_heights = c(1.5, 0.5, 1), subplots = 1:3, pvalue_table = FALSE, 
          ES_geom = "line", ymin = NULL, ymax = NULL) 
{
  ...
  # AG addition to force y limits
  if(!(any(c(is.null(ylim), is.null(ymax))))){
    p.res <- p.res + ylim(ymin, ymax)  
  }
